...
**一个 Web 产品，里面包含 RAG、Workflow、Memory，以及模拟面试场景下的轻 Agent 能力。**

可以理解为三层：

1. 产品层：`Web 页面 / 用户流程 / 数据记录`
2. AI 能力层：`JD 解析`、`知识检索问答`、`评分与复盘`
3. Agent 层：只在 `模拟面试` 模块中承担多轮追问和动态调整
...
flowchart TD
    ...
    F --> G["7. 实现最小闭环 3<br/>模拟配置 -> 多轮问答 -> 评分反馈"]
    G --> H["8. 实现最小闭环 4<br/>练习记录 -> 薄弱项聚合 -> 推荐下一步"]
...
- 明确 MVP 只做 `JD 解析`、`知识库问答`、`模拟面试`、`练习记录与薄弱项追踪`
